...
type SystemItem = { id: string | number; name: string; scenario: string; status: SystemState; progress: number; lastEvent: string; destination: string; provider?: string; monthlyCost?: number };
type SessionUser = { id: string; email: string; name: string; picture: string | null };
type DashboardData = { metrics: { delivered: number; active: number; attention: number }; setup: { progress: number; completedSteps: number }; systems: Array<{ id: string; name: string; goals_json: string; provider: string; monthly_cost: number; status: string; progress: number; last_event: string; destination: string }>; recent: Array<{ id: string; event_type: string; status: string; created_at: number }> };
...
const setupSteps = [
  ...
  { label: "Review cost", help: "Approve the monthly resources and setup fee." },
...
function Brand({ inverted = false }: { inverted?: boolean }) {
  return <span className={`brand ${inverted ? "brand-inverted" : ""}`}><Image className="brand-mark" src="/icons/icon-48.png" width={31} height={31} alt="" priority /><span>Whatsnot</span></span>;
}
    ...
    <main>
      <section className="hero">
        <div className="hero-copy">
          ...
          <div className="trust-row"><span>✓ ₹650/month starter infrastructure</span><span>✓ One-time implementation fee</span><span>✓ No markup on messages</span></div>
        </div>
      ...
      <section className="public-section pricing-section" id="pricing">
        <div className="price-copy"><span className="kicker">TRANSPARENT BY DESIGN</span><h2>₹650 per month for starter infrastructure.<br />Implementation is paid once.</h2><p>One monthly infrastructure estimate covers all the business goals selected in the setup. Whatsnot’s implementation and testing fee is separate and charged once.</p><ul><li><span>✓</span>Select orders, appointments, payments and support together</li><li><span>✓</span>No WhatsApp message markup from Whatsnot</li><li><span>✓</span>Monthly provider costs remain visible</li></ul></div>
        <div className="price-card"><span>TYPICAL CLOUDFLARE STARTER</span><div className="price-total"><strong>₹650</strong><small>per month</small></div><div className="price-row"><span>Cloud infrastructure</span><b>₹650/month</b></div><div className="price-row"><span>Whatsnot implementation & testing</span><b>₹5,849 once</b></div><div className="price-row muted"><span>Message markup</span><b>₹0</b></div><p>This is an estimate. Actual provider and Meta charges depend on usage and are shown before real activation.</p><button className="button primary large full" onClick={() => navigate("signup")}>Build my estimate <span>→</span></button></div>
      </section>
...
function AuthPage({ mode, navigate }: { mode: "login" | "signup"; navigate: (route: Route) => void }) {
  const [googleReady, setGoogleReady] = useState(false);
  const [checked, setChecked] = useState(false);
  const queryError = typeof window === "undefined" ? "" : new URLSearchParams(window.location.search).get("error");
  useEffect(() => {
    fetch("/api/auth/config", { credentials: "same-origin", cache: "no-store" })
      .then(response => response.ok ? response.json() : Promise.reject())
      .then((config: { google?: boolean }) => setGoogleReady(Boolean(config.google)))
      .catch(() => setGoogleReady(false))
      .finally(() => setChecked(true));
  }, []);
  return <div className="auth-page"><div className="auth-aside"><button className="brand-button" onClick={() => navigate("home")}><Brand inverted /></button><div><span className="kicker light">YOU’LL ALWAYS KNOW WHAT COMES NEXT</span><h1>{mode === "signup" ? "Turn your first notification idea into a tested system." : "Welcome back to your notification control centre."}</h1><FlowLine steps={["Account", "Cost approval", "WhatsApp", "Hosting", "Live test"]} active={mode === "signup" ? 0 : 1} /></div><p>Your sign-in is verified by Google on the server. Whatsnot never stores your Google password.</p></div><main className="auth-main"><div className="auth-card"><div className="mobile-auth-brand"><Brand /></div><span className="kicker">{mode === "signup" ? "CREATE YOUR ACCOUNT" : "SIGN IN"}</span><h2>{mode === "signup" ? "Start securely with Google." : "Continue your setup."}</h2><p>{mode === "signup" ? "After sign-in, you can choose several goals and review monthly costs before activation." : "Use the Google account connected to your Whatsnot profile."}</p>{queryError && <div className="form-error" role="alert">Sign-in could not be completed. Please try again or check the Cloudflare setup.</div>}<button className="button primary large full google-auth" disabled={!checked || !googleReady} onClick={() => { window.location.href = `/api/auth/google/start?returnTo=${encodeURIComponent(mode === "signup" ? "/checkout" : "/dashboard")}`; }}>G&nbsp;&nbsp; {checked ? googleReady ? "Continue with Google" : "Google sign-in needs setup" : "Checking sign-in…"}</button>{!googleReady && checked && <div className="auth-config-note"><strong>Sign-in is safely unavailable.</strong><p>Add the included D1 database and Google credentials in Cloudflare to enable it. This button will never bypass authentication.</p></div>}<div className="auth-divider"><span>other methods</span></div><div className="social-auth"><button disabled title="Email/password authentication is not configured">Email & password — unavailable</button><button disabled title="GitHub authentication is not configured">GitHub — unavailable</button></div><p className="auth-switch">{mode === "signup" ? "Already have an account?" : "New to Whatsnot?"} <button onClick={() => navigate(mode === "signup" ? "login" : "signup")}>{mode === "signup" ? "Sign in" : "Create an account"}</button></p></div></main></div>;
}
...
function CheckoutPage({ navigate, onComplete }: { navigate: (route: Route) => void; onComplete: () => void }) {
  ...
  const monthly = provider === "cloudflare" ? 650 : 1350;
  const setupFee = volume === "starter" ? 5849 : volume === "growing" ? 7049 : 9099;
  return <div className="checkout-page"><header><button className="brand-button" onClick={() => navigate("home")}><Brand /></button><span><b>Secure review</b><small>Nothing is deployed until you approve</small></span></header><main><section className="checkout-form"><span className="kicker">STEP 2 OF 5 · COST APPROVAL</span><h1>Choose a sensible starting size.</h1><p>The provider amount is a recurring monthly infrastructure estimate. Whatsnot’s implementation and testing fee is charged once.</p><div className="choice-section"><h2>Expected notifications each month</h2><div className="choice-grid">{[{id:"starter",title:"Up to 10,000",body:"Small shop or clinic"},{id:"growing",title:"10,000–100,000",body:"Growing online business"},{id:"scale",title:"100,000+",body:"High-volume operations"}].map(option => <button className={volume === option.id ? "selected" : ""} onClick={() => setVolume(option.id)} key={option.id}><span>{volume === option.id ? "✓" : ""}</span><strong>{option.title}</strong><small>{option.body}</small></button>)}</div></div><div className="choice-section"><h2>Preferred resource provider</h2><div className="provider-grid">{[{id:"cloudflare",logo:"CF",title:"Cloudflare",body:"Estimated ₹650 per month"},{id:"railway",logo:"RW",title:"Railway",body:"Estimated ₹1,350 per month"}].map(option => <button className={provider === option.id ? "selected" : ""} onClick={() => setProvider(option.id)} key={option.id}><i>{option.logo}</i><span><strong>{option.title}</strong><small>{option.body}</small></span><b>{provider === option.id ? "✓" : ""}</b></button>)}</div></div><div className="info-banner"><span>i</span><p><strong>What “resources” means:</strong> the computing, storage and traffic allowance that keeps your notification system running every month. Actual provider usage can vary.</p></div></section><aside className="order-summary"><span className="kicker">CLEAR MONTHLY + ONE-TIME COSTS</span><h2>Starter activation</h2><div className="summary-row"><span>Estimated provider resources<small>{provider === "cloudflare" ? "Cloudflare Worker, storage and event capacity" : "Railway service, database and event capacity"}</small></span><b>₹{monthly.toLocaleString("en-IN")}/month</b></div><div className="summary-row"><span>Implementation & testing<small>One-time connection guidance, templates and verification</small></span><b>₹{setupFee.toLocaleString("en-IN")} once</b></div><div className="summary-row free"><span>Whatsnot message markup</span><b>₹0</b></div><div className="summary-total"><span>Due for the first month<small>Monthly resources + one-time setup; taxes shown before real payment</small></span><strong>₹{(monthly + setupFee).toLocaleString("en-IN")}</strong></div><label className="consent"><input type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)} /><span>I understand the provider estimate repeats monthly and the implementation fee is paid once. This demonstration does not process payment.</span></label><button disabled={!accepted} className="button primary large full" onClick={() => { onComplete(); navigate("dashboard"); }}>Approve demo estimate <span>→</span></button><button className="text-button centered" onClick={() => navigate("learn")}>Review the tutorial again</button><p className="secure-note">🔒 No card details are collected in this frontend prototype.</p></aside></main></div>;
}

function AppShell({ route, navigate, userName, children, unread, openNotifications, openCommand, mobileOpen, setMobileOpen, collapsed, setCollapsed }: { route: AppRoute; navigate: (route: Route) => void; userName: string; children: React.ReactNode; unread: number; openNotifications: () => void; openCommand: () => void; mobileOpen: boolean; setMobileOpen: (value: boolean) => void; collapsed: boolean; setCollapsed: (value: boolean) => void }) {
  const current = navItems.find(item => item.route === route)!;
  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}><a className="skip-link" href="#app-content">Skip to content</a><aside className={`app-sidebar ${mobileOpen ? "open" : ""}`}><div className="sidebar-brand"><Brand inverted /><button className="desktop-collapse" onClick={() => setCollapsed(!collapsed)} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} title={collapsed ? "Expand sidebar" : "Collapse sidebar"}>{collapsed ? "›" : "‹"}</button><button className="mobile-close" onClick={() => setMobileOpen(false)} aria-label="Close navigation">×</button></div><div className="org-card"><span>{userName.slice(0, 2).toUpperCase()}</span><div><strong>Your business</strong><small>Notification account</small></div><b>⌄</b></div><nav aria-label="Application navigation">{navItems.map(item => <button title={`${item.label} — ${item.hint}`} className={route === item.route ? "active" : ""} onClick={() => { navigate(item.route); setMobileOpen(false); }} key={item.route}><span>{item.icon}</span><div><strong>{item.label}</strong><small>{item.hint}</small></div>{item.route === "setup" && <i>NEW</i>}</button>)}</nav><div className="sidebar-help"><span>?</span><div><strong>Words feel confusing?</strong><p>Open the plain-language guide.</p></div><button onClick={() => navigate("settings")}>View guide</button></div><div className="sidebar-user"><span>{userName.slice(0, 2).toUpperCase()}</span><div><strong>{userName}</strong><small>Account owner</small></div><button aria-label="User options">•••</button></div></aside>{mobileOpen && <button className="sidebar-scrim" aria-label="Close navigation" onClick={() => setMobileOpen(false)} />}<div className="app-area"><header className="app-topbar"><div><button className="mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open navigation">☰</button><span className="top-icon">{current.icon}</span><div><strong>{current.label}</strong><small>{current.hint}</small></div></div><div><button className="search-button" onClick={openCommand}><span>⌕</span> Find anything <kbd>Ctrl K</kbd></button><button className="top-button" onClick={openNotifications} aria-label={`${unread} unread notifications`}>♢{unread > 0 && <i>{unread}</i>}</button><button className="button primary small top-setup" onClick={() => navigate("setup")}>+ Set up notification</button></div></header><main id="app-content" className="app-content">{children}</main></div></div>;
}

function ProgressBoard({ navigate, progress }: { navigate: (route: Route) => void; progress: number }) {
  const completed = Math.min(5, Math.floor(progress / 20));
  const labels = ["Goals selected", "WhatsApp connected", "Hosting selected", "Cost approved", "Live message test"];
  const details = ["Choose one or several goals", "Link a Meta business number", "Choose where it runs", "Review monthly and one-time costs", "Verify the complete journey"];
  const steps = labels.map((title, index) => ({ title, detail: details[index], state: index < completed ? "done" : index === completed ? "current" : "next" }));
  const remaining = Math.max(0, 5 - completed);
  return <section className="panel progress-board"><div className="panel-heading"><div><span className="kicker">YOUR SETUP JOURNEY</span><h2>{remaining === 0 ? "Your notification setup is complete." : `${remaining} step${remaining === 1 ? "" : "s"} left before your first live notification.`}</h2><p>This tracker updates from your saved setup progress, so completed, current and remaining stages stay clear.</p></div><div className="progress-ring" style={{ "--progress": `${progress * 3.6}deg` } as React.CSSProperties}><strong>{progress}%</strong><span>complete</span></div></div><div className="status-flow">{steps.map((item, index) => <button className={item.state} onClick={() => navigate("setup")} key={item.title}><span>{item.state === "done" ? "✓" : index + 1}</span><div><strong>{item.title}</strong><small>{item.detail}</small></div>{index < steps.length - 1 && <i />}</button>)}</div><div className="next-action"><span>▶</span><div><strong>{remaining ? `Next action: ${labels[completed]}` : "All setup stages completed"}</strong><p>{remaining ? details[completed] : "Open your notification systems to monitor activity."}</p></div><button className="button primary" onClick={() => navigate(remaining ? "setup" : "systems")}>{remaining ? "Continue setup" : "View systems"} →</button></div></section>;
}

function DashboardPage({ navigate, userName, data, loading }: { navigate: (route: Route) => void; userName: string; data: DashboardData | null; loading: boolean }) {
  const metrics = data?.metrics ?? { delivered: 0, active: 0, attention: 0 };
  const progress = data?.setup.progress ?? 0;
  const recent = data?.recent ?? [];
  return <><div className="app-page-heading"><div><span className="kicker">YOUR NOTIFICATION CONTROL CENTRE</span><h1>Hello, {userName} 👋</h1><p>{loading ? "Loading your saved account data…" : "Here is your real setup status and the clearest next action."}</p></div><button className="button primary" onClick={() => navigate("setup")}>+ Set up notification</button></div><ProgressBoard navigate={navigate} progress={progress} /><section className="dashboard-grid"><div className="panel simple-metrics"><div><span className="metric-symbol green">✓</span><p>Messages delivered</p><strong>{metrics.delivered.toLocaleString("en-IN")}</strong><small>Recorded for your account</small></div><div><span className="metric-symbol blue">↗</span><p>Active notification journeys</p><strong>{metrics.active}</strong><small>Currently running</small></div><div><span className="metric-symbol orange">!</span><p>Needs your attention</p><strong>{metrics.attention}</strong><small>Items requiring action</small></div></div><div className="panel understand-card"><span>PLAIN-LANGUAGE GUIDE</span><h2>What is a “notification system”?</h2><p>It is one complete connection that notices a business event—like a new order—and safely sends the right WhatsApp message.</p><button onClick={() => navigate("learn")}>See the 3-minute explanation →</button></div></section><section className="dashboard-grid lower"><div className="panel recent-panel"><div className="panel-title"><div><h2>Latest message activity</h2><p>Only events recorded for your signed-in account appear here.</p></div><button onClick={() => navigate("activity")}>View all</button></div>{recent.length ? recent.map(item => <div className="activity-item" key={item.id}><span className={item.status === "delivered" || item.status === "read" ? "green" : "blue"}>{item.status === "delivered" || item.status === "read" ? "✓" : "↻"}</span><div><strong>{item.event_type}</strong><small>{item.status} · {new Date(item.created_at * 1000).toLocaleString()}</small></div><button aria-label={`Open ${item.event_type}`}>›</button></div>) : <div className="empty-inline"><strong>No message activity yet</strong><p>Events will appear after you connect WhatsApp and send a live test.</p></div>}</div><div className="panel quick-start"><div className="panel-title"><div><h2>Choose your business goals</h2><p>You can select several goals in one setup.</p></div></div>{(Object.keys(scenarios) as ScenarioKey[]).map(key => <button onClick={() => navigate("setup")} key={key}><span>{scenarios[key].icon}</span><div><strong>{scenarios[key].label}</strong><small>{scenarios[key].summary}</small></div><b>›</b></button>)}</div></section></>;
}
...
function SystemsPage({ systems, setSystems, navigate, toast }: { systems: SystemItem[]; setSystems: (systems: SystemItem[]) => void; navigate: (route: Route) => void; toast: (title: string, body: string) => void }) {
  ...
  const pause = async (item: SystemItem) => { const next: SystemState = item.status === "Paused" ? "Live" : "Paused"; const response = await fetch(`/api/systems/${encodeURIComponent(String(item.id))}`, { method: "PATCH", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ status: next === "Live" ? "live" : "paused" }) }); if (!response.ok) { toast("Change not saved", "The system status could not be updated."); return; } setSystems(systems.map(system => system.id === item.id ? { ...system, status: next } : system)); setMenu(null); toast(next === "Paused" ? "Notification system paused" : "Notification system resumed", `${item.name} is now ${next.toLowerCase()}.`); };
...
function SetupPage({ systems, setSystems, navigate, toast }: { systems: SystemItem[]; setSystems: (systems: SystemItem[]) => void; navigate: (route: Route) => void; toast: (title: string, body: string) => void }) {
  const [step, setStep] = useState(0);
  const [selectedGoals, setSelectedGoals] = useState<ScenarioKey[]>(["orders"]);
  const [connected, setConnected] = useState(false);
  const [provider, setProvider] = useState("Cloudflare");
  const [name, setName] = useState("Business notifications");
  const [saving, setSaving] = useState(false);
  const monthlyCost = provider === "Cloudflare" ? 650 : provider === "Railway" ? 1350 : 0;
  const chosenLabels = selectedGoals.map(key => scenarios[key].label);
  const toggleGoal = (key: ScenarioKey) => setSelectedGoals(current => current.includes(key) ? current.length === 1 ? current : current.filter(value => value !== key) : [...current, key]);
  const next = () => setStep(value => Math.min(4, value + 1));
  const launch = async () => {
    setSaving(true);
    try {
      const response = await fetch("/api/systems", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ name, goals: chosenLabels, provider, monthlyCost }) });
      const result = await response.json() as { id?: string; error?: string };
      if (!response.ok || !result.id) throw new Error(result.error || "Setup could not be saved.");
      const newSystem: SystemItem = { id: result.id, name, scenario: chosenLabels.join(", "), status: "Setting up", progress: 20, lastEvent: "Waiting for WhatsApp connection", destination: "Not connected", provider, monthlyCost };
      setSystems([...systems, newSystem]);
      toast("Setup saved", `${name} is now tracked in your account.`);
      navigate("systems");
    } catch (error) {
      toast("Setup was not saved", error instanceof Error ? error.message : "Please try again.");
    } finally { setSaving(false); }
  };
  return <><div className="app-page-heading setup-heading"><div><span className="kicker">GUIDED SETUP</span><h1>Set up WhatsApp notifications</h1><p>Choose several business goals, see every stage, and approve the monthly cost before anything goes live.</p></div><button className="button ghost" onClick={() => navigate("systems")}>Save & exit</button></div><section className="setup-layout"><aside className="setup-nav"><div className="setup-percent"><div><strong>{step * 20}%</strong><span>{5 - step} stages left</span></div><progress value={step} max="5" /></div>{setupSteps.map((item, index) => <button className={step === index ? "active" : step > index ? "done" : ""} onClick={() => index <= step && setStep(index)} key={item.label}><span>{step > index ? "✓" : index + 1}</span><div><strong>{item.label}</strong><small>{item.help}</small></div></button>)}<div className="setup-help"><span>?</span><p><strong>Need a person?</strong><br />Guided assistance can be added before activation.</p><button onClick={() => toast("Support is not connected yet", "Add a support booking provider before launch.")}>Learn more</button></div></aside><div className="setup-panel"><div className="journey-tracker"><div><span>YOUR CHOSEN JOURNEY</span><strong>{step * 20}% complete · {5 - step} stages left</strong></div><FlowLine steps={setupSteps.map(item => item.label)} active={step} /></div>{step === 0 && <div className="setup-step"><span className="step-label">STEP 1 OF 5</span><h2>Which business updates should WhatsApp handle?</h2><p>Select one or several goals. All selected goals share the same ₹650/month Cloudflare starting infrastructure estimate.</p><div className="goal-grid">{(Object.keys(scenarios) as ScenarioKey[]).map(key => <button aria-pressed={selectedGoals.includes(key)} className={selectedGoals.includes(key) ? "selected" : ""} onClick={() => toggleGoal(key)} key={key}><span>{scenarios[key].icon}</span><div><strong>{scenarios[key].label}</strong><small>{scenarios[key].summary}</small></div><b>{selectedGoals.includes(key) ? "✓" : "+"}</b></button>)}</div><div className="selected-goals"><strong>{selectedGoals.length} goal{selectedGoals.length === 1 ? "" : "s"} selected</strong>{chosenLabels.map(label => <span key={label}>✓ {label}</span>)}</div></div>}{step === 1 && <div className="setup-step"><span className="step-label">STEP 2 OF 5</span><h2>Connect your WhatsApp Business account.</h2><p>Meta provides the secure connection. Whatsnot never asks for your Facebook password.</p><div className={`connection-card ${connected ? "connected" : ""}`}><div className="meta-logo">∞</div><div><h3>{connected ? "Demonstration connection selected" : "Meta Embedded Signup"}</h3><p>{connected ? "This preview is not a real WhatsApp connection." : "The real Meta connection requires server credentials before launch."}</p></div><button className={`button ${connected ? "ghost" : "primary"}`} onClick={() => setConnected(value => !value)}>{connected ? "Remove preview" : "Preview connection state"}</button></div><div className="permission-list"><h3>What you will approve</h3><div><span>✓</span><p><strong>Send approved templates</strong><small>Only from the number you select.</small></p></div><div><span>✓</span><p><strong>Receive delivery updates</strong><small>So you can see delivered or failed messages.</small></p></div><div><span>✓</span><p><strong>Revoke access</strong><small>Disconnect from Whatsnot or Meta at any time.</small></p></div></div><div className="warning-note">Demonstration mode: no Meta account is connected and no message can be sent.</div></div>}{step === 2 && <div className="setup-step"><span className="step-label">STEP 3 OF 5</span><h2>Where should the notification system run?</h2><p>“Hosting” means the computing and storage that keep your automation available each month.</p><div className="hosting-options">{[{name:"Cloudflare",badge:"Recommended",desc:"Fast global events and beginner-friendly deployment",cost:"Estimated ₹650 per month"},{name:"Railway",badge:"Managed",desc:"Application and database hosting in one place",cost:"Estimated ₹1,350 per month"},{name:"My own cloud",badge:"Advanced",desc:"Use infrastructure your business already owns",cost:"Monthly cost calculated after connection"}].map(option => <button className={provider === option.name ? "selected" : ""} onClick={() => setProvider(option.name)} key={option.name}><span>{option.name.slice(0, 2).toUpperCase()}</span><div><b>{option.badge}</b><h3>{option.name}</h3><p>{option.desc}</p><small>{option.cost}</small></div><i>{provider === option.name ? "✓" : ""}</i></button>)}</div><div className="allocation-map"><h3>Resources included for all selected goals</h3><div><span>⚡</span><p><strong>Event receiver</strong><small>Notices approved business events</small></p><b>1</b></div><div><span>⇄</span><p><strong>Reliable message queue</strong><small>Keeps bursts from being lost</small></p><b>1</b></div><div><span>▣</span><p><strong>Secure settings store</strong><small>Protects connection references</small></p><b>1</b></div></div></div>}{step === 3 && <div className="setup-step"><span className="step-label">STEP 4 OF 5</span><h2>Review goals, journey and costs.</h2><p>The monthly infrastructure amount and one-time implementation fee are shown separately.</p><label className="setup-name">Notification system name<input value={name} onChange={event => setName(event.target.value)} /></label><div className="review-card"><div><span>Business goals</span><strong>{chosenLabels.join(", ")}</strong></div><div><span>WhatsApp connection</span><strong>{connected ? "Demonstration only" : "Not connected"}</strong></div><div><span>Resource provider</span><strong>{provider}</strong></div><div><span>Expected setup time</span><strong>10–20 minutes after real approvals</strong></div></div><div className="cost-review"><div><span>Estimated provider cost</span><strong>{monthlyCost ? `₹${monthlyCost.toLocaleString("en-IN")}/month` : "Calculated after connection"}</strong></div><div><span>Implementation & testing</span><strong>₹5,849 once</strong></div><p>Selected goals do not each add another ₹650. Actual Meta and provider usage remains visible separately.</p></div></div>}{step === 4 && <div className="setup-step go-live-step"><span className="step-label">STEP 5 OF 5</span><div className="launch-visual"><span>✓</span></div><h2>Ready to save “{name}”.</h2><p>This saves the selected goals and provider to your authenticated account. It does not allocate resources or send messages until the real Meta and provider integrations are completed.</p><div className="launch-flow"><FlowLine steps={["Validate choices", "Connect Meta", "Allocate resources", "Send test", "Confirm delivery"]} active={1} /></div><div className="launch-summary"><span>✓ {selectedGoals.length} goals included</span><span>✓ ₹{monthlyCost.toLocaleString("en-IN")}/month estimate</span><span>✓ Deactivation remains available</span></div><button disabled={saving} className="button primary large" onClick={launch}>{saving ? "Saving…" : "Save setup to my account →"}</button></div>}<div className="setup-footer"><button className="button ghost" disabled={step === 0} onClick={() => setStep(value => value - 1)}>← Back</button>{step < 4 && <button className="button primary" disabled={step === 1 && !connected} onClick={next}>Continue to {setupSteps[step + 1].label.toLowerCase()} →</button>}</div></div></section></>;
}
...
export function WhatsnotApp() {
  ...
  useEffect(() => {
    const hydrate = window.setTimeout(async () => {
      ...
      setCollapsedState(localStorage.getItem("whatsnot-sidebar-collapsed") === "true");
      let sessionUser: SessionUser | null = null;
      try {
        const response = await fetch("/api/session", { credentials: "same-origin", cache: "no-store" });
        if (response.ok) sessionUser = (await response.json() as { user: SessionUser }).user;
      } catch { /* signed-out is a valid state */ }
      setUser(sessionUser);
      if ((appRoutes.includes(initialRoute as AppRoute) || initialRoute === "checkout") && !sessionUser) {
      ...
      if (sessionUser) {
        try {
          ...
          if (response.ok) {
            ...
            setDashboard(data);
            setSystemsState(data.systems.map(item => ({ id: item.id, name: item.name, scenario: JSON.parse(item.goals_json).join(", "), status: item.status === "live" ? "Live" : item.status === "paused" ? "Paused" : "Setting up", progress: item.progress, lastEvent: item.last_event, destination: item.destination, provider: item.provider, monthlyCost: item.monthly_cost })));
          }
        } catch { /* the authenticated empty state remains visible */ }
      }
  ...
  const navigate = (next: Route) => {
    if ((appRoutes.includes(next as AppRoute) || next === "checkout") && !user && !sessionLoading) next = "login";
    window.history.pushState({}, "", routePath(next)); setRoute(next); window.scrollTo({ top: 0, behavior: "smooth" });
  };
  const setSystems = (next: SystemItem[]) => setSystemsState(next);
  const setCollapsed = (value: boolean) => { setCollapsedState(value); localStorage.setItem("whatsnot-sidebar-collapsed", String(value)); };
  ...
  if (sessionLoading && (appRoutes.includes(route as AppRoute) || route === "checkout")) return <div className="session-loader"><Brand /><p>Checking your secure session…</p></div>;
  if (route === "home") return <HomePage navigate={navigate} />;
  if (route === "learn") return <LearnPage navigate={navigate} />;
  if (route === "login" || route === "signup") return <AuthPage mode={route} navigate={navigate} />;
  ...
  return <AppShell route={appRoute} navigate={navigate} userName={user?.name || "Account"} unread={0} openNotifications={() => setNotifications(true)} openCommand={() => setCommand(true)} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} collapsed={collapsed} setCollapsed={setCollapsed}>
    {appRoute === "dashboard" && <DashboardPage navigate={navigate} userName={user?.name || "there"} data={dashboard} loading={sessionLoading} />}
